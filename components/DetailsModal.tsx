...
import { ShareIcon, StarIcon, TrashIcon } from '../constants';
...
const DetailsModal: React.FC<DetailsModalProps> = ({ post, user, isAdmin, onClose, onAddComment, onRate, onShareClick, onDelete }) => {
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <div className="text-sm flex-grow">
            <div className="flex justify-between items-start">
                <div>
                    <p className="font-semibold text-lg text-amber-900">{post.brewery}</p>
                    <p className="text-gray-600">{post.type} - {post.nation}</p>
                    <p className="text-gray-600">{post.abv.toFixed(1)}% ABV</p>
                </div>
            ...
            <p className="text-2xl font-bold text-amber-800 mt-2">€{post.price.toFixed(2)}</p>
            <p className="text-gray-700 italic mt-2">"{post.description}"</p>
          </div>
        </div>

        <div className="border-t pt-4">
          <h4 className="font-semibold text-md text-gray-800 mb-2">Valuta questa birra</h4>
